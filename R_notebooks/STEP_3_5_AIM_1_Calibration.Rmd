...
```{r}
gene <-  'PTCD1' #'SOX11' #  'SEMA5A'  #
qplot(x=dfa[,gene], y=dfb_f[,gene], color=dfa$cluster.group, ylab='RNA-seq', xlab='Array', main=gene)
```
...
save(genelist, file='data/feature_pairs_female_v3_round3.rda')
```
...
### PULLING OUT SOME GENES
#setwd("E:/Work/GBM_clusters/Classifier_Workflow")
resdf_m <- read.csv('data/feature_pairs_males_v3.csv')
dfa_m <- read.csv('data/Males_CV_Data_v3.csv')
dfb_m <- read.csv('data/Males_RNASeq_Data_v3.csv')
...
genelist <-  c(
  colnames(arr_m)[cluster1],
  colnames(arr_m)[cluster2],
  colnames(arr_m)[cluster3],
  colnames(arr_m)[cluster4],
  colnames(arr_m)[cluster5]
)

save(genelist, file='data/feature_pairs_male_v3_round3.rda')
```
