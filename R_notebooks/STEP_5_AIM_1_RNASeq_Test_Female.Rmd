...
lapply(gpairs, function(x) x[! (x %in% colnames(tcga_f ))] ) # missing "FABP5"  "CD24"   "LGALS3"
...
# gs <- c(unlist(unique(genepairs_cl)), "Barcode", "ClusterLabel")
# 
# d1 <- dat_f[,gs]
# d2 <- val_f
# which(colnames(val_f) %in% c('cluster.group','value'))
# colnames(d2)[c(19429, 19428)] <- c('Barcode','ClusterLabel')
# d2 <- d2[,gs]
# d2$Barcode <- str_to_lower(d2$Barcode)
# dmerge <- rbind(d1,d2)
# 
...
params <- list(max_depth=6,   # "height" of the tree, 6 is actually default. I think about 12 seems better.  (xgboost parameter)
               ...
               gamma=1.5,      # Minimum loss req'd to again partition a leaf node. higher number ~ more conservative (xgboost)
               lambda=2.5,     # L2 regularization term on weights, higher number ~ more conservative (xgboost parameter)
               alpha=1.5,      # L1 regularization term on weights. higher number ~ more conservative (xgboost parameter)
               verbose=0,
               train_perc=0.8,
...
resdf_val <- resdf[str_detect(resdf$SampleIDs,pattern='TCGA'),]

table(Label=resdf_val$Label, Call=resdf_val$BestCalls)
