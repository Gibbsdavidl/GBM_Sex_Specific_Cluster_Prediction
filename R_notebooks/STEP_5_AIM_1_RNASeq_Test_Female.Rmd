...
val_f <- read_csv('../data/Females_RNASeq_Data.csv.gz')
...
resdf1 <- data.frame(SampleIDs=mod_f$train_sample_ids, ClusterLabel=mod_f$train_label)


resdf2 <- data.frame(SampleIDs=mod_f$results()$SampleIDs, Pred=mod_f$results()$BestCall, IDs2=mod_f$test_sample_ids)


resdf <- inner_join(resdf1, resdf2)


table(resdf$ClusterLabel, resdf$Pred)
