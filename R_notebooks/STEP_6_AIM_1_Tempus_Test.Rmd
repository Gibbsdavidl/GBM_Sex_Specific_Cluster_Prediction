...
load('../results/genepair_features/females_genepairs_rna_refined.rda')
...
resdf$Months <- round(resdf$Survival / 30)

modfit <- survfit(Surv(Survival, CensoredCode)~BestCalls,data=resdf)
surp <- ggsurvplot(modfit, pval = T, xlim=c(0,1700))
surp

modfit <- survfit(Surv(Months, CensoredCode)~BestCalls,data=resdf)
surp <- ggsurvplot(modfit, pval = T, pval.coord = c(1, 0))
surp


ggsave("../figures/survplot_tempus_F_genepairs_refined_rna_V2.pdf", surp$plot, height = 8, width = 8)
#ggsave("../figures/survplot_tempus_F_init_genepairs.pdf", surp$plot, height = 8, width = 8)
...
load('../results/genepair_features/males_genepairs.rda')
#load('../results/genepair_features/males_genepairs_val_refined.rda')
...
ggsave("../figures/survplot_tempus_M_with_genepairs_v2.pdf", surp$plot, height = 8, width = 8)

#ggsave("../figures/survplot_tempus_M_with_female_features.pdf", surp$plot, height = 8, width = 8)

jdf <- inner_join(resdf, tempus.clin, by = join_by('SampleIDs'=='Study.ID'))
