...
# the sample sheet that links file id to case id
samples <- read_tsv('C:/Users/daveg/Desktop/cptac3-gbm/rnaseq/gdc_sample_sheet.2024-04-12.tsv')

biospecimen <- read_tsv('C:/Users/daveg/Desktop/cptac3-gbm/biospecimen.cohort.2024-04-12 (1)/sample.tsv')

clinical <- read_tsv('C:/Users/daveg/Desktop/cptac3-gbm/clinical.cohort.2024-04-12 (1)/clinical.tsv')

# gather all the downloaded RNA-seq count files.
rnafiles <- list.files('C:/Users/daveg/Desktop/cptac3-gbm/rnaseq/',pattern = 'gene_counts.tsv',full.names = T,recursive = T)
rnafiles <- rnafiles[ !str_detect(rnafiles, pattern = 'parcel') ]

# these are the column names of each RNA-seq file
cnames <- c('gene_id',	'gene_name',	'gene_type',	'unstranded',	'stranded_first',	
...
for (fi in rnafiles) {
  ...
  bits <- str_split(fi, pattern = '\\.')[[1]]
  id <- str_split(bits[1], pattern = '\\/')[[1]][7]
  
  # read in the counts and filter to just protein coding genes
  cnts <- read_tsv(fi,skip = 6,col_names = cnames)
  cnts <- cnts %>% dplyr::filter(gene_type == 'protein_coding')
  
  caselist[[i]] <- samples$`Case ID`[samples$`File ID` == id]
  samplelist[[i]] <- samples$`Sample ID`[samples$`File ID` == id]
  
  #qplot(cnts$unstranded, cnts$tpm_unstranded, ylim=c(0,20000),xlim=c(0,20000))
  #cor(cnts$unstranded, cnts$tpm_unstranded, method = 'spearman')
  #[1] 0.9470363

  newdf <- data.frame(cnts$unstranded,row.names = cnts$gene_id)  # was tpm_unstranded
  rnalist[[i]] <- newdf 
  
  i <- i+1
}

# bind the expression profiles and transpose it
rnadf <- do.call('cbind', rnalist)

# get the unique case IDs some profiles are from multiple samples, but same case ID
caselistuniq <- sapply(unlist(caselist), function(a) unique(str_squish(str_split(a,',')[[1]])))

# get the sample IDs out
samplelistuniq <- as.character(
  unlist( 
    sapply(unlist(samplelist), function(a) unique(str_squish(str_split(a,',')[[1]]))) ) )

# annotate the rows and columns
colnames(rnadf) <- caselistuniq
rnadf$Symbol <- cnts$gene_name

# observe the duplicated symbols
# most have all zeros for one entry and are tied to Y chromosome expr
didx <- which(duplicated(rnadf$Symbol))

# sum the repeated genes
rnadf_agg <- aggregate(.~Symbol,data=rnadf,FUN=sum) 

# transpose 
rnadf_agg_t <- as.data.frame(t(rnadf_agg[,-1]))
colnames(rnadf_agg_t) <- rnadf_agg$Symbol



this_biospecimen <- biospecimen[biospecimen$sample_submitter_id %in% samplelistuniq,]

#There are two normal samples "C3N-01197" and "C3N-03452"
which(rownames(rnadf_agg_t) %in% c("C3N-01197", "C3N-03452"))
# 53 73

rna_agg_df_tumor <- rnadf_agg_t[!(rownames(rnadf_agg_t) %in% c("C3N-01197", "C3N-03452")), ]


biodf <- biospecimen[biospecimen$sample_submitter_id %in% samplelistuniq,]
biodf[biodf$sample_type == 'Primary Tumor',]

clinsub <- clinical[clinical$case_submitter_id %in% rownames(rna_agg_df_tumor), ]
cidx <- match( rownames(rna_agg_df_tumor), clinsub$case_submitter_id )

all(clinsub$case_submitter_id[cidx] == rownames(rna_agg_df_tumor))
# True

rna_agg_df_tumor$CaseID <- clinsub$case_submitter_id[cidx]
rna_agg_df_tumor$Sex <- clinsub$gender[cidx]
#rna_agg_df_tumor$Survival <- clinsub$days_to_death[cidx]
rna_agg_df_tumor$Survival <- clinsub$days_to_last_known_disease_status
rna_agg_df_tumor$Censored <- clinsub$vital_status[cidx]

#write.csv(rna_agg_df_tumor,'../data/cptac3_gbm_full_rnaseq_table.csv',quote = F,row.names = F)

rna_f <- rna_agg_df_tumor[rna_agg_df_tumor$Sex == 'female',]
rna_m <- rna_agg_df_tumor[rna_agg_df_tumor$Sex == 'male',]
write.csv(rna_f,'../data/cptac3_gbm_rnaseq_table_unstranded_Female.csv.gz', quote = F,row.names = F)
write.csv(rna_m,'../data/cptac3_gbm_rnaseq_table_unstranded_Male.csv.gz', quote = F,row.names = F)

write.csv(biodf, '../data/cptac3_biospecimen_unstranded_table.csv')
