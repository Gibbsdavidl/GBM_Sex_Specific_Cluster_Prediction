...
run_model <- function(train, test, genepairs_list, params, npairs) {
  ...
  if (npairs > 0) {
    ...
    genepairs_cl <- clean_genepairs_list(genepairs_cl, test)
  } else {
    genepairs_cl <- genepairs_list
  }
...
mod_f <- run_model(train,test,genepairs_cl,params1)
...
ggsave("../figures/survplot_validation_refined_features_F_8by8_gset4.pdf", surp$plot, height = 8, width = 8)
...
write.csv(df, file = 'results/feature_importance/tcga_array_gset4.csv')
...
# run the model
mod_f2 <- run_model(val_test_dat,train,genepairs_cl,params1)
...
gset4[['cluster3']] <- gset4[['cluster3']][1:8]
...
# run the model
mod_t <- run_model(train,test,gset3,params1,10)

# check the preds
table(mod_t$results()$BestCall)

# cluster1 cluster2 cluster3 cluster4 cluster5 
# 47        3       10        3       23 

# 10 pairs and 6 tree depth
#cluster1 cluster2 cluster3 cluster4 cluster5 
#44        6       10        8       18 

# build the predicted output table
resdf <- data.frame(Survival=mod_t$test_data$Survival, 
                    Censored=mod_t$test_data$Censored, 
                    Barcode=mod_t$test_data$Barcode,
                    Pred=mod_t$results()$BestCall)
head(resdf)
...
surp <- ggsurvplot(modfit, pval = T, xlim=c(0,1700) )
ggsave("figures/survplot_tempus_F_gset4_10pairs_6depth.pdf", surp$plot, height = 8, width = 8)

write.csv(resdf, file="results/pred_tempus_F_gset4_10pairs_6depth.csv")
...
# run the model
mod_t <- run_model(train,test,gset4,params1,0)
