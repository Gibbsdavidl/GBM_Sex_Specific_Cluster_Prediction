...
write.csv(resdf, file='../results/F_validation_predictions_using_rna_pairs.csv')

  #          ASTROCYTOMA GBM MIXED OLIGODENDROGLIOMA UNKNOWN
  # cluster1           3  16     0                 0       6
  # cluster2           0   3     0                 0       1
  # cluster3          21   4     3                18       7
  # cluster4           8  14     0                 2       1
  # cluster5           5  21     1                 3       4
...
Compare, before and after parameter sets.
...
load(file='../data/f_genepairs.rda')

load("E:/Work/GBM_clusters/GBM_JIVE_ANALYSIS/data/F_processed_data.rda")

genepairs1 <- fgenepairs$median_min
genepairs2 <- fgenepairs$rna


gpairs <- shorter_plist(genepairs1, 12)

# clean the pairs
genepairs_cl <- clean_genepairs_list(gpairs, dat_f)  # only one with missing genes
genepairs_cl <- clean_genepairs_list(gpairs, tcga_f)  # only one with missing genes
...
test_dat <- mod_f$test_data[, c('Barcode','ClusterLabel')]
test_dat$BestCall <- mod_f$results()$BestCall

pheatmap::pheatmap(
table(Label=test_dat$ClusterLabel, Pred=test_dat$BestCall),
cluster_rows = F, cluster_cols = F,display_numbers = T,width = 3,height = 4
,fontsize = 14,filename = '../figures/heatmap_f_rna_genepairs_comparison_before.pdf'
)
...
gpairs <- shorter_plist(gp1, 24)

# clean the pairs
genepairs_cl <- clean_genepairs_list(gpairs, dat_m)  # only one with missing genes
genepairs_cl <- clean_genepairs_list(gpairs, rna_m)  # only one with missing genes
...
# run the model
mod_m$predict(data_frame = rna_m, 
              label_name = 'ClusterLabel',
              sample_id = 'Barcode')
...
pheatmap::pheatmap(
table(Label=cl_test, Pred=cl_pred),
cluster_rows = F, cluster_cols = F,display_numbers = T,width = 3,height = 4
,fontsize = 14,filename = '../figures/heatmap_m_rna_genepairs_comparison_before.pdf'
)
