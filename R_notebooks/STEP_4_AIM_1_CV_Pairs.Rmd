...
```{r}
dat.m <- read_csv('../data/jive_training_array_data_M_v2.csv')
...
train <- dat.m[idx,1:14455]
test <- dat.m[jdx,1:14455]
...
clean_genepairs_list <- function(genepairs, dat) {
  
  genepairs_clean <- list()
  i <- 1
  for (pi in names(genepairs)) {
    genes <- genepairs[[pi]]
    missing_genes <- unique( genes [! genes %in% colnames(dat_f)] )
    newpairlist <- c()
    for (j in seq.int(from=1,to=length(genes), by=2)) {
      if ( (genes[i] %in% missing_genes) | (genes[i+1] %in% missing_genes) ) {
        # then don't add them!
        print(paste0("removing from pair list:  ", genes[i], ' ', genes[i+1]))
      } else {
        newpairlist <- c(newpairlist, genes[i], genes[i+1])
      }
    }
...
idx <- sample(1:139, size=100, replace = F)
jdx <- setdiff(1:139, idx)
train <- dat_f[idx,1:14455]
test <- dat_f[jdx,1:14455]
