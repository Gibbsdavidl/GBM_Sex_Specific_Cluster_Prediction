...
genepairs <- m_genepairs$min_median
...
mod_m$autocv(data_frame=dat_m,
            ...
            sample_id = 'Barcode',
            drop_list='Sex',
            data_mode=c('sigpairs'), # pairs, allpairs, sigpairs, quartiles, tertiles, binarize, ranks, original #
            signatures=sigs_m, #sigs.m,
            pair_list=sigs_m, #gpairs,  # subset to these genes.
            params=params,
            cv_rounds=10
            )
...
write.csv(as.data.frame(df.m), '../results/male_tcga_cv_pairs_only.csv', sep=',', quote = F, row.names = F)
...
glist <- shorter_plist(fgenepairs$median_min, 10)
...
save(buffy, file='../results/models/F_tcga_array_CV.rda')
```
